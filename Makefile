...
dumps/swh-archiver.dump:
	@if ! echo | $(PSQL) $(DBNAME2) ; then echo "Can't find $(DBNAME2). Try make -C ../swh-archiver/sql/ distclean filldb" ; false ; fi
	pg_dump -F custom --no-owner $(DBNAME2) > $@
dumps/swh-archiver.sql:
	@if ! echo | $(PSQL) $(DBNAME2) ; then \
		echo "Can't find $(DBNAME2). Try make -C ../swh-archiver/sql/ distclean filldb" ; \
		false ; \
	fi
